...
const authenicate = async (token, needData = false) => {
    let result;
    const statement = "SELECT ?? FROM ?? WHERE email = ? AND student_id = ? LIMIT 1",
          fields = ["student_id", "email", "first_name", "last_name", "curriculum_id"],
          tables = ["student"],
    ...
    if (needData) {
        try {
            ...
            result = await connection.query(statement, [fields, tables, decode.email, decode.student_id]);
